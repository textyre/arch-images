...
source "qemu" "archlinux" {
  # Input: official Arch Linux cloud image (qcow2), fetched fresh each build
  iso_url      = var.cloud_image_url
  iso_checksum = var.cloud_image_checksum
  disk_image   = true   # input is a disk image, not an ISO
  ...
  # Cloud-init seed: virtual CD-ROM with user-data + meta-data
  cd_files = [
...
build {
  ...
  # Ansible: upgrade → keyring → aur_tools → vagrant_user → cleanup
  provisioner "ansible" {
